...
class COCODataset(JointsDataset):
    ...
    def _get_ann_file_keypoint(self):
        ...
        return os.path.join(
            self.root,
            #'annotations/'+ self.image_set.split('/')[0]+'/',
            # prefix + '_' + self.image_set.split('/')[1] + '.json'
            'annotations',
    ...
    def image_path_from_index(self, index):
        ...
        prefix = 'test2017' if 'test' in self.image_set else self.image_set#.split('/')[1]

        data_name = prefix + '.zip@' if self.data_format == 'zip' else prefix

        image_path = os.path.join(
            # self.root, 'images/'+self.image_set.split('/')[0]+'/', data_name, file_name)
